...
with open( 'voting_models.pkl', 'rb' ) as f:
    models = load( f )
...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        if model['search_spaces']:
            search = BayesSearchCV( estimator = model['model'], search_spaces = model[ 'search_spaces' ], cv = cv, n_iter=10, n_jobs=-1, scoring='roc_auc', verbose=5 )
            start = time()
        ...
        if model['search_spaces']:
            search = BayesSearchCV( estimator = model['model'], search_spaces = model[ 'search_spaces' ], cv = cv, n_iter=20, n_jobs=-1, scoring='roc_auc', verbose=5 )
            start = time()
